...
export default function AccountsPage() {
  ...
  const arStats = {
    total: filteredReceivables.reduce((sum, ar) => sum + ar.outstandingAmount, 0),
    current: filteredReceivables.filter(ar => ar.status === 'current').reduce((sum, ar) => sum + ar.outstandingAmount, 0),
    overdue: filteredReceivables.filter(ar => ar.status === 'overdue').reduce((sum, ar) => sum + ar.outstandingAmount, 0),
    count: filteredReceivables.length,
    overdueCount: filteredReceivables.filter(ar => ar.status === 'overdue').length
  };

  const apStats = {
    total: filteredPayables.reduce((sum, ap) => sum + ap.outstandingAmount, 0),
    current: filteredPayables.filter(ap => ap.status === 'current').reduce((sum, ap) => sum + ap.outstandingAmount, 0),
    overdue: filteredPayables.filter(ap => ap.status === 'overdue').reduce((sum, ap) => sum + ap.outstandingAmount, 0),
    count: filteredPayables.length,
    overdueCount: filteredPayables.filter(ap => ap.status === 'overdue').length
  };

  const filteredData = selectedTab === 'receivables' ?
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Manage customer receivables, vendor payables, and debt collection processes
        </p>
      </div>
                  ...
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div className="space-y-1">
                        <CardTitle className="text-lg">{receivable.customerName}</CardTitle>
                        <CardDescription>Invoice: {receivable.invoiceNumber}</CardDescription>
                      </div>
                      <div className="flex items-center gap-2">
                        <Badge variant={getStatusColor(receivable.status, receivable.daysPastDue)}>
                          <StatusIcon className="h-3 w-3 mr-1" />
                          {receivable.status === 'overdue' ? `${receivable.daysPastDue} days overdue` : 'Current'}
                        </Badge>
                      </div>
                    </div>
                  </CardHeader>
                    ...
                    <div className="space-y-4">
                      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                        <div className="space-y-1">
                          ...
                          <p className="font-bold text-lg text-green-600">{formatCurrency(receivable.outstandingAmount)}</p>
                        </div>
                        <div className="space-y-1">
                          <p className="text-xs text-muted-foreground">Invoice Date</p>
                          <p className="text-sm font-medium">{receivable.invoiceDate}</p>
                        </div>
                        <div className="space-y-1">
                          <p className="text-xs text-muted-foreground">Due Date</p>
                          <p className="text-sm font-medium">{receivable.dueDate}</p>
                        </div>
                        <div className="space-y-1">
                          <p className="text-xs text-muted-foreground">Credit Terms</p>
                          <p className="text-sm font-medium">{receivable.creditTerms}</p>
                        </div>
                      </div>
                      ...
                      {receivable.lastPaymentDate && (
                        <div className="p-3 bg-green-50 rounded-lg">
                          <p className="text-sm text-green-700">
                            Last Payment: {formatCurrency(receivable.lastPaymentAmount)} on {receivable.lastPaymentDate}
                          </p>
                        </div>
                      )}
                      ...
                      <div className="flex items-center justify-between pt-4 border-t">
                        <div className="text-sm text-muted-foreground">
                          Plant: {receivable.plantId.split('-')[1].toUpperCase()}
                        </div>
              ...
              return (
                <Card key={payable.id}>
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div className="space-y-1">
                        <CardTitle className="text-lg">{payable.vendorName}</CardTitle>
                        <CardDescription>Invoice: {payable.invoiceNumber}</CardDescription>
                      </div>
                      <div className="flex items-center gap-2">
                        <Badge variant="outline">{payable.category}</Badge>
                        <Badge variant={getStatusColor(payable.status, payable.daysPastDue)}>
                          <StatusIcon className="h-3 w-3 mr-1" />
                          {payable.status === 'overdue' ? `${payable.daysPastDue} days overdue` : 'Current'}
                        </Badge>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                        <div className="space-y-1">
                          <p className="text-xs text-muted-foreground">Outstanding Amount</p>
                          <p className="font-bold text-lg text-red-600">{formatCurrency(payable.outstandingAmount)}</p>
                        </div>
                        <div className="space-y-1">
                          <p className="text-xs text-muted-foreground">Invoice Date</p>
                          <p className="text-sm font-medium">{payable.invoiceDate}</p>
                        </div>
                        <div className="space-y-1">
                          <p className="text-xs text-muted-foreground">Due Date</p>
                          <p className="text-sm font-medium">{payable.dueDate}</p>
                        </div>
                        <div className="space-y-1">
                          <p className="text-xs text-muted-foreground">Payment Terms</p>
                          <p className="text-sm font-medium">{payable.paymentTerms}</p>
                        </div>
                      </div>
                      ...
                      {payable.lastPaymentDate && (
                        <div className="p-3 bg-blue-50 rounded-lg">
                          <p className="text-sm text-blue-700">
                            Last Payment: {formatCurrency(payable.lastPaymentAmount)} on {payable.lastPaymentDate}
                          </p>
                        </div>
                      )}
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Age analysis of outstanding customer receivables</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {[
                    { range: "0-30 days", amount: 27750000, count: 2, color: "text-green-600" },
                    { range: "31-60 days", amount: 19800000, count: 2, color: "text-yellow-600" },
                    { range: "61-90 days", amount: 0, count: 0, color: "text-orange-600" },
                    { range: "90+ days", amount: 0, count: 0, color: "text-red-600" }
                  ].map((bucket) => (
                    <div key={bucket.range} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                      <div>
                      ...
                      <div className="text-right">
                        <p className={`font-bold text-lg ${bucket.color}`}>{formatCurrency(bucket.amount)}</p>
                        <p className="text-xs text-muted-foreground">
                          {bucket.amount > 0 ? ((bucket.amount / arStats.total) * 100).toFixed(1) + '%' : '0%'}
                        </p>
                      </div>
                    </div>
                  ))}
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Age analysis of outstanding vendor payables</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {[
                    { range: "0-15 days", amount: 45500000, count: 1, color: "text-green-600" },
                    { range: "16-30 days", amount: 65650000, count: 2, color: "text-yellow-600" },
                    { range: "31-45 days", amount: 8750000, count: 1, color: "text-orange-600" },
                    { range: "45+ days", amount: 0, count: 0, color: "text-red-600" }
                  ].map((bucket) => (
                    <div key={bucket.range} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                      <div>
                        <p className="font-medium">{bucket.range}</p>
                      ...
                      <div className="text-right">
                        <p className={`font-bold text-lg ${bucket.color}`}>{formatCurrency(bucket.amount)}</p>
                        <p className="text-xs text-muted-foreground">
                          {bucket.amount > 0 ? ((bucket.amount / apStats.total) * 100).toFixed(1) + '%' : '0%'}
                        </p>
                      </div>
                    </div>
                  ))}
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Largest outstanding receivables</CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    .map((customer) => (
                      <div key={customer.id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">{customer.invoiceNumber}</p>
                        </div>
                        <div className="text-right">
                          <p className="font-bold text-green-600">{formatCurrency(customer.outstandingAmount)}</p>
                          <Badge variant={getStatusColor(customer.status, customer.daysPastDue)} className="text-xs">
                            {customer.status}
                          </Badge>
                        </div>
                      </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Largest outstanding payables</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {filteredPayables
                    ...
                    .map((vendor) => (
                      <div key={vendor.id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                        ...
                        <div className="text-right">
                          <p className="font-bold text-red-600">{formatCurrency(vendor.outstandingAmount)}</p>
                          <Badge variant={getStatusColor(vendor.status, vendor.daysPastDue)} className="text-xs">
                            {vendor.status}
                          </Badge>
                        </div>
                      </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="text-center p-4 bg-green-50 rounded-lg">
                      ...
                      <p className="text-2xl font-bold text-green-600">{formatCurrency(arStats.total)}</p>
                      <p className="text-xs text-green-600">{arStats.count} invoices</p>
                    </div>
                    <div className="text-center p-4 bg-red-50 rounded-lg">
                      <p className="text-sm text-red-700">Expected Outflow</p>
                      <p className="text-2xl font-bold text-red-600">{formatCurrency(apStats.total)}</p>
                      <p className="text-xs text-red-600">{apStats.count} payments</p>
                    </div>
                  </div>
                  <div className="text-center p-4 bg-blue-50 rounded-lg border-2 border-blue-200">
                    ...
                    <p className="text-xs text-blue-600">
                      {arStats.total > apStats.total ? 'Positive cash flow expected' : 'Negative cash flow expected'}
                    </p>
                  </div>
                </div>
              </CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Important financial ratios and KPIs</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      <p className="text-muted-foreground">DSO (Days Sales Outstanding)</p>
                      <p className="text-xl font-bold">
                        {Math.round((arStats.total / (138000000 / 30)))} days
                      </p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">DPO (Days Payable Outstanding)</p>
                      <p className="text-xl font-bold">
                    ...
                    <div>
                      ...
                      <p className="text-xl font-bold text-green-600">
                        {(((arStats.total - arStats.overdue) / arStats.total) * 100).toFixed(1)}%
                      </p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">Payment Compliance</p>
                      <p className="text-xl font-bold text-blue-600">
                        {(((apStats.total - apStats.overdue) / apStats.total) * 100).toFixed(1)}%
                      </p>
                    </div>
                  </div>
                </div>
